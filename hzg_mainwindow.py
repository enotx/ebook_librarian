...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # 在搜索结果中右键打开菜单
        self.treeShowResult.customContextMenuRequested.connect(self.openResultContextMenu)
    ...
    def add_search_result(self, model, file_name, file_path, file_size, file_id, file_hash):
        model.insertRow(0)
    ...
    def create_search_result_model(self, query_result):
        ...
        for i in query_result:
            self.add_search_result(model, str(i[1]), str(i[2]), int(i[4]), int(i[0]), str(i[5]))
        return model
    ...
    def search_by_keyword(self, keyword, page = 1, option = ""):
        ...
        if option == "init":
            # 连接数据库
            self.dbcursor = db_connection.retrieve_file_table_by_keyword(max(keyword.split(" "), key=len))
            a = self.dbcursor[0]
            # 为分页数据申请存储空间
            self.search_history_list = [[]]
        ...
        if len(self.search_history_list[page-1]) == RESULT_NUMBER:
            ...
            self.buttonResultNext.setEnabled(False)
        self.treeShowResult.setModel(self.create_search_result_model(self.search_history_list[page-1]))
    ...
    def openResultContextMenu(self,position):
        ...
        if indexes:
            ...
            actionOpenContainingFolder = menu.addAction(self.tr("Open Containing Folder"))
            ...
            action = menu.exec_(self.treeShowResult.viewport().mapToGlobal(position))
            if action == actionOpenFile:
                current_index_file_path = indexes[1].data() + "/" + indexes[0].data() 
                hzg_fileinfo.open_file(current_index_file_path)
            elif action == actionOpenContainingFolder:
                current_index_folder_path = indexes[1].data()
                hzg_fileinfo.open_file(current_index_folder_path)
            elif action == actionFindDuplicates:
                db_buffer = hzg_database(DATABASE_DIRECTORY, BASIC_DATABASE_NAME)
                ...
                if a == 1:
                    ...
                    self.treeShowResult.setModel(self.create_search_result_model(duplicate_file_list))
                    self.treeShowResult.setColumnWidth(0,250)
                    self.treeShowResult.setColumnWidth(1,400)
                    self.treeShowResult.resizeColumnToContents(3)
    ...
    def get_current_selected_file_info(self):
        pass
    ...
    def on_toolSelectScanDir_clicked(self):
        t = QtWidgets.QFileDialog.getExistingDirectory(self, "Select Directory", self.directory_to_scan)
        if t!="":
    ...
    def on_buttonScan_clicked(self):
        ...
        if os.path.exists(a) and os.path.isdir(a):
            if self.checkboxInitScan.isChecked():
                self.scan_warning_dialog = hzg_dialog.WarningDialog(title="Hello",
    ...
    def scan_directory(self, dir_path, option):
        self.scan_progressbar_dialog = hzg_dialog.ProgressDialog(title="Scanning...")
        self.scan_progressbar_dialog.show()
        self.scan_thread = hzg_workers.ScannerWorker(self)
        self.scan_thread.setup(dir_path,option)
        a = self.scan_thread.get_file_num()
        # self.scan_thread.trigger2.connect(self.scan_progressbar_dialog.setup_progress_bar)
    ...
    def scan_thread_update_info(self, x):
        ...
        if not self.scan_flag:
            ...
            self.statusBar.showMessage(str(x[0]+x[1]) + " files scanned." +str(x[0])+" imported and "+ str(x[1]) + " ignored")
    ...
    def on_actionPreference_triggered(self):
        # tabdialog = hzg_dialog.PreferenceDialog()
        pass
